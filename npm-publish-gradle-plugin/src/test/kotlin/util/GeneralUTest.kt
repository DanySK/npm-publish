...
class GeneralUTest : UTest() {
  @TestFactory
  fun notFalseTests(): Collection<DynamicTest> =
    mapOf(
        "true" to true,
        "TRUE" to true,
        "y" to true,
  ...
  @TestFactory
  fun npmFullNameTests(): Collection<DynamicTest> =
    listOf(Triple("scope", "package", "@scope/package"), Triple(null, "package", "package")).map {
      (scope, packageName, expected) ->
      DynamicTest.dynamicTest(
        "::npmFullName converts scope '$scope' & packageName '$packageName' to '$expected'"
      ) {
