...
public abstract class NodeExecTask : DefaultTask(), PluginLogger {
  ...
  init {
    nodeHome.convention(
      project.layout.projectDirectory.dir(project.providers.environmentVariable("NODE_HOME"))
    )
    node.convention(
      nodeHome.file(
  ...
  protected fun exec(args: Collection<String?>, config: Action<ExecSpec> = Action {}): ExecResult =
    execOps.exec {
      val cmd = args.toTypedArray().filterNotNull()
      info { "Executing: ${cmd.joinToString(" ")}" }
      @Suppress("SpreadOperator") it.commandLine(*cmd.toTypedArray())
      config.execute(it)
    }
  ...
  public fun nodeExec(args: Collection<String?>, config: Action<ExecSpec> = Action {}): ExecResult =
    exec(listOf(node.get().asFile.absolutePath) + args, config)
}
