...
internal fun Project.configure(packages: NpmPackages): Unit =
  with(PluginLogger.wrap(logger)) {
    ...
    packages.whenObjectAdded {
      ...
      val assTask =
        tasks
          .register(assembleTaskName(it.name), NpmAssembleTask::class.java) { task ->
      ...
      tasks
        .register(packTaskName(it.name), NpmPackTask::class.java) { task ->
    ...
    packages.whenObjectRemoved {
      ...
      tasks.names
        .filter { name -> name.startsWith("publish${it.name.capitalised()}PackageTo") }
